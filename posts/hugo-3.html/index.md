...
**如果想设置默认不展示文本内容，可以设置`open=false`，还可以设置标题**：
...
> 2024.04.22更新：由于hugo-search-fuse-js项目更新换代了，使用时需要安装其他的依赖，不满足开箱即用的需求，我将最原始最简单的版本上传到了新建的仓库里[lewky/hugo-search-fuse-js](https://github.com/lewky/hugo-search-fuse-js)，下文都基于该原始版本进行教程！！

### 安装搜索插件hugo-search-fuse-js到本地

`hugo-search-fuse-js`并不是一个单独的主题，而是Hugo主题的一个组件：
1. 下载[hugo-search-fuse-js](https://github.com/lewky/hugo-search-fuse-js)到站点的主题目录`/themes/hugo-search-fuse-js`下，**注意，目录名必须是`hugo-search-fuse-js`**
...
上述这种方式创建的page页面作为友链，有个缺点，就是无法使用目录。
